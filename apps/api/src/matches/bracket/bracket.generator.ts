...
export class BracketGenerator {
  ...
  async generateInitialRosters(
    ...
    await this.createEmptyBracket(1, roundAmount);
  }

  async createEmptyBracket(
    roundId: number,
    totalRounds: number,
    startDate: Date = new Date(),
  ) {
    // Store matchups by round for parent reference
    const matchupsByRound: { [round: number]: any[] } = {};

    // Start from the final round (1) and work backwards to the first round
    for (let currentRound = totalRounds; currentRound >= 1; currentRound--) {
      const matchupsInThisRound = [];

      // Calculate how many matchups are in this round
      const matchupCount = Math.pow(2, totalRounds - currentRound);

      // For each matchup in this round
      for (let i = 0; i < matchupCount; i++) {
        // Determine parent matchup if not in the final round
        const parentMatchupId =
          currentRound < totalRounds
            ? matchupsByRound[currentRound + 1][Math.floor(i / 2)].id
            : undefined;

        // Calculate start date for this matchup
        const matchupStartDate = new Date(
          startDate.getTime() +
        ...
        matchupsInThisRound.push(createdMatchup[0]);
      }

      // Store this round's matchups for reference in the next iteration
      matchupsByRound[currentRound] = matchupsInThisRound;
    }

    return matchupsByRound[1]; // Return the first round matchups
  }
}
